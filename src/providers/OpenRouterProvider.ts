...
export class OpenRouterProvider implements AiProvider {
  ...
  async generate(prompt: string, system: string, model: string): Promise<string> {
    ...
    if (res.status !== 200) throw new Error(`OpenRouter error: ${requestUrlError(res)}`);
  ...
  async stream(
    ...
    if (!this.apiKey) throw new Error("No OpenRouter API Key");
    const res = await window.fetch("https://openrouter.ai/api/v1/chat/completions", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${this.apiKey}`,
    ...
    if (!res.ok) throw new Error(`OpenRouter stream error: ${await fetchError(res)}`);
