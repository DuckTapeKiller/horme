...
export class DiagnosticService {
  ...
  async getIndexHealth(): Promise<IndexHealth[]> {
    ...
    health.push(
      await this.checkFolderStatus(
        ...
        "vault",
        vaultPath,
        this.plugin.vaultIndexer.index.length,
        this.plugin.vaultIndexer.isLoaded,
      ),
    );
    ...
    health.push(
      await this.checkIndexStatus("Tags", "tags", tagsPath, this.plugin.tagIndexer.entryCount, true),
    );
    ...
    health.push(
      await this.checkIndexStatus(
        ...
        "grammar",
        grammarPath,
        this.plugin.grammarIndexer.chunks.length,
        true,
      ),
    );
  ...
  private async checkFolderStatus(
    name: string,
    id: string,
    folderPath: string,
    ...
    try {
      ...
      if (!isLoaded)
        return { name, id, status: "loading", lastUpdate: latestMtime, entryCount: 0, path: folderPath };
