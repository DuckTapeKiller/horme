 ...
 * Each model family has its own prefix convention:
 *  - nomic-embed-text: "search_query: " / "search_document: "
 *  - mxbai-embed-*:    "Represent this sentence for searching relevant passages: " / ""
 *  - all-minilm, etc:  "" / "" (symmetric — no prefix)
 */
function getModelPrefixes(model: string): { query: string; document: string } {
  ...
  // Symmetric models (all-minilm, bge-small, etc.) — no prefix
...
export class VaultIndexer {
    ...
    this.plugin.onSettingsChange(() => {
      ...
      if (this.indexedModel && this.indexedModel !== current) {
        ...
        this.deleteAllShards().catch(e =>
          console.error("Horme Brain: Failed to delete stale shard files after model change.", e)
        );
  ...
  private async loadShardedIndex(): Promise<void> {
    ...
    while (true) {
      ...
      try {
        ...
        console.error(`Horme Brain: Failed to read shard ${shardIndex}, stopping load.`, e);
        break;
      }
      ...
      try {
        ...
        if (errorCount > 50) {
          new HormeErrorModal(
            ...
            `Indexing stopped after ${errorCount} consecutive failures. This usually means Ollama is unreachable or the embedding model is not loaded. Check that Ollama is running and that the model "${this.plugin.settings.ragEmbeddingModel}" is available.`,
   ...
   */
  async search(query: string, topN = 8): Promise<string[]> {
    ...
    if (!this.plugin.settings.vaultBrainEnabled || !canAccess) {
      if (!canAccess) console.warn("Horme Privacy Guard: Vault search blocked (Cloud Provider Active & allowCloudRAG is OFF).");
      return [];
    }
    ...
    try {
      ...
      const searchTerms = refinedQuery.split(/\s+/).filter(t => t.length > 2);
      
      // 2. Multi-query: full query + keyword-distilled variant
      const { query: qPrefix } = getModelPrefixes(this.plugin.settings.ragEmbeddingModel);
      const primaryEmbedding = await this.plugin.embeddingService.getEmbedding(`${qPrefix}${refinedQuery}`);
      
      const keyTerms = searchTerms.filter(t => t.length > 3).join(' ');
      const secondaryEmbedding = keyTerms && keyTerms !== refinedQuery
        ? await this.plugin.embeddingService.getEmbedding(`${qPrefix}${keyTerms}`)
        : null;
      const MIN_SIMILARITY = 0.20;

      // 3. Score all entries: max(primary, secondary) + keyword bonus
      const scored = this.index
        .map(entry => {
          ...
          // Keyword boosting across all metadata fields
          let keywordBonus = 0;
      ...
      // 4. Fetch content for top results
