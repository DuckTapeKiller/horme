...
export class TagIndexer {
  ...
  constructor(plugin: HormePlugin) {
    ...
    this.indexPath = normalizePath(
      `${configDir}/plugins/${this.plugin.manifest.id}/Tags Index/tag-index.json`,
    );
    void this.loadIndex();
  }
      ...
      if (exists) {
        ...
        if (this.indexedModel && this.indexedModel !== currentModel) {
          this.plugin.debugLog(
            `Horme Tags: Model changed (${this.indexedModel} → ${currentModel}). Index cleared.`,
          );
          this.index = [];
      ...
      for (let i = 0; i < tagList.length; i += BATCH_SIZE) {
        ...
        try {
          ...
          this.plugin.diagnosticService.report(
            "Tags",
            `Tag batch indexing failed: ${errorToMessage(e)}`,
            "warning",
          );
        }
      }
