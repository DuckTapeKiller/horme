...
export class FactCheckResultModal extends Modal {
  ...
  constructor(app: App, plugin: HormePlugin, resultMarkdown: string, editor: Editor, selectionEnd: { line: number, ch: number }) {
    super(app);
    this.plugin = plugin;
  ...
  private renderClaimBlock(containerEl: HTMLElement, claim: ClaimBlock) {
    ...
    insertBtn.addEventListener("click", () => {
       try {
           ...
           const footnoteContent = `**Fact Check - ${claim.verdict}**: ${claim.note} (Source: ${claim.source})`.replace(/\n/g, " ");
           ...
           this.editor.replaceRange(`\n\n[^${index}]: ${footnoteContent}`, { line: lastLine, ch: lastLineLength });
