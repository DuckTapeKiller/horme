...
export class GrammarScholarSkill implements Skill {
  ...
  description =
    "Consults high-authority grammar and orthography manuals for precision checks on false friends, syntax, and orthotypography.";
  ...
  parameters: SkillParameter[] = [
    {
      ...
      type: "string",
      description:
        "Specific words or grammatical constructions to verify (e.g., 'eventualmente', 'subjunctive').",
      required: true,
    },
    {
  ...
  async execute(params: unknown): Promise<string> {
    try {
      ...
      if (!terms)
        return `Invalid parameters for ${this.name}: expected {"terms": string, "context_sentence"?: string}.`;
